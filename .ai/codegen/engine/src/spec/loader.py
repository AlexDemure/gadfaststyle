...
def load_knowledge(path: Path) -> KnowledgeSpec:
    data = load_yaml(path)
    return KnowledgeSpec(
        version=int(data["version"]),
        shared_documents=list(data.get("shared_documents", [])),
        agent_required_documents=list(data.get("agent_required_documents", [])),
        agent_optional_documents=list(data.get("agent_optional_documents", [])),
    )
...
def load_project_spec(spec_dir: Path) -> ProjectSpec:
    return ProjectSpec(
        ...
        knowledge=load_knowledge(spec_dir / "knowledge.yaml"),
    )
