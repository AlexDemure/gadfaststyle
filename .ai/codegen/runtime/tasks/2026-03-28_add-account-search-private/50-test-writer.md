...
Добавлены интеграционные тесты для приватной ручки `POST /api/-/accounts:search`.

Файл:

- `tests/test_integrations/test_entrypoints/test_http/test_system/test_accounts/test_search.py`
...
Failed because `pytest` is not installed in the environment.
