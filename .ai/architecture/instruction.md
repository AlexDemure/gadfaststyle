...
2. Определи, какие зоны проекта затрагивает задача.
3. Прочитай соответствующие файлы:
   - `.ai/architecture/src/instruction.md`
   - `.ai/architecture/tests/instruction.md`
4. Если задача затрагивает usecase, продолжи чтение по дереву:
   - `.ai/architecture/src/application/usecases/instruction.md`
   - инструкция конкретной операции внутри `.ai/architecture/src/application/usecases/`
5. Только после этого открывай конкретные файлы в `src/` и `tests/`.
...
- Если после удаления операции или сценария в доменной папке не остается рабочих файлов, удаляй и саму папку домена.
- Не оставляй пустые `registry.py`, `__init__.py` и пустые зеркальные тестовые папки после удаления функциональности.
- После удаления доменной папки обнови связанные `registry.py`, `__init__.py` и импорты на уровень выше.
